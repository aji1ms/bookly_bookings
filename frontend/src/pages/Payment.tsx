...
interface PaymentUserState {
    _id: string;
}
export default function PaymentPage() {
    const navigate = useNavigate();
    const location = useLocation();
    const bookingState = location.state?.booking as PaymentBookingState;
    const userState = location.state?.user as PaymentUserState;
    ...
    const handleCardChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const errorKey = name as keyof CardErrors;
    ...
    const validatePayment = (): boolean => {
        ...
        if (!/^\d{2}\/\d{2}$/.test(expiry)) {
            ...
            const [month, year] = expiry.split("/").map(Number) as [number, number];
            const now = new Date();
            ...
            <main className="max-w-5xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-5 gap-12 mt-8">
                <div className="lg:col-span-3 space-y-10">
                    ...
                    <form onSubmit={handlePayment} className="space-y-6">
                        ...
                        <button
                            type="submit"
                            className="w-full cursor-pointer bg-gray-900 text-white py-6 rounded-3xl font-bold text-lg hover:bg-black transition-all shadow-xl hover:-translate-y-1 flex items-center justify-center gap-3 group dark:bg-white dark:text-gray-900 dark:hover:bg-gray-900 dark:hover:text-white dark:border-gray-400 border border-gray-900"
                        >
                            {loading ? (
