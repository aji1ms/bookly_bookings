            ...
            <div className="max-w-6xl mx-auto">
                <div className="bg-gray-900 dark:bg-gray-800 rounded-3xl px-12 py-16 text-center relative overflow-hidden">
                    ...
                    <h2 className="relative font-serif-display text-3xl md:text-5xl font-normal text-white leading-tight tracking-tight mb-4">
                        {t("cta.title_main")}<br />
                        is <em className="text-white/50">{t("cta.title_italic")}</em>
                    </h2>
                    <p className="relative text-base text-white/50 leading-relaxed max-w-sm mx-auto mb-10">
                        {t("cta.description")}
                    </p>
                    <div className="relative flex flex-wrap gap-3 justify-center">
                        <Link to={"/services"} className="inline-flex items-center gap-2 text-sm font-semibold text-gray-900 bg-white px-8 py-4 rounded-full no-underline hover:-translate-y-0.5 hover:shadow-2xl transition-all duration-200">
                            {t("cta.btn_primary")}
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
                                <path d="M3 8h10M8 3l5 5-5 5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                        </Link>
                        <Link to={"/services"} className="inline-flex items-center text-sm font-medium text-white/70 border border-white/20 px-7 py-3.5 rounded-full no-underline hover:text-white hover:border-white/50 hover:bg-white/10 transition-all duration-200">
                            {t("cta.btn_secondary")}
                        </Link>
                    </div>
                </div>
